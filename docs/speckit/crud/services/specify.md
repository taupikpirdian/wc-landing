# Speckit — Specify: CRUD Layanan (Filament)

## 1. Purpose
Menyediakan fitur manajemen layanan pada panel admin menggunakan Filament. Admin dapat membuat, melihat, mengubah, dan menghapus data layanan, termasuk upload gambar.

---

## 2. Actors
- **Admin**: pengguna yang memiliki akses ke dashboard Filament.

---

## 3. Functional Requirements

### 3.1 List Layanan
- Admin dapat melihat daftar layanan dalam tabel.
- Kolom yang ditampilkan:
  - ID  
  - Title  
  - Label
  - Description
  - Image Icon (icon)  
  - Image Cover (thumbnail)  
  - Created At  
- Fitur:
  - Search berdasarkan `title`
  - Sort by `created_at`
  - Pagination default Filament

---

### 3.2 Create Layanan
- Admin dapat menambahkan layanan baru.
- Field yang diisi:
  - **Title** (required)
  - **Label** (optional)
  - **Description** (optional)
  - **Image Icon** (optional, upload → stored under `services/`)
  - **Image Cover** (required, upload → stored under `services/`)
- Validasi:
  - Title wajib
  - Image Icon optional dan harus berupa file jpeg/png
  - Image Cover wajib dan harus berupa file jpeg/png
  - Ukuran maksimal 2MB
- Setelah submit, data tersimpan ke tabel `services`.

---

### 3.3 Edit Layanan
- Admin dapat mengubah:
  - Title  
  - Label (optional)
  - Description  
  - Image Icon (opsional untuk diganti)
  - Image Cover (opsional untuk diganti)
- Jika image diganti:
  - File lama dihapus
  - File baru disimpan

---

### 3.4 Delete Layanan
- Admin dapat menghapus layanan.
- Mendukung single delete dan bulk delete.
- Ketika data dihapus:
  - File gambar terkait ikut dihapus dari storage.

---

### 3.5 View Layanan Detail (Opsional)
- Menampilkan detail lengkap layanan.
- Menggunakan Filament View Page.

---

## 4. Non-Functional Requirements

### 4.1 Performance
- Query harus efisien, minimal penggunaan eager load.
- Upload file menggunakan storage default (`public`).
- Compress image sebelum disimpan.

### 4.2 Security
- Akses hanya untuk user yang sudah login di panel Filament.
- Validasi input mengikuti standar Filament Forms.

### 4.3 UX/UI
- Menggunakan komponen standar Filament:
  - `TextInput`
  - `Textarea`
  - `FileUpload`
- Preview gambar muncul pada list dan detail page.

---

## 5. Data Structure Mapping

| Table Column | Form Field | Notes |
|--------------|------------|-------|
| id | auto | handled by DB |
| title | TextInput | required |
| label | TextInput | optional |
| desc | Textarea | optional |
| image_icon | FileUpload | optional |
| image_cover | FileUpload | required on create |
| created_at | auto | otomatis |
| updated_at | auto | otomatis |

---

## 6. API / Events
CRUD dilakukan hanya melalui panel Filament.  
Tidak ada endpoint API tambahan.

---

## 7. Acceptance Criteria

### Create
- [ ] Dapat membuat layanan baru  
- [ ] Image Icon dan Image Cover berhasil di-upload ke folder `services/`  
- [ ] Validasi berjalan sesuai aturan  

### Edit
- [ ] Dapat mengubah Title, Label, dan Description  
- [ ] Dapat mengganti gambar  
- [ ] Gambar lama terhapus saat diganti  

### Delete
- [ ] Single delete & bulk delete berjalan  
- [ ] File gambar ikut terhapus  

### List
- [ ] Pagination berfungsi  
- [ ] Search by title berfungsi  
- [ ] Sort by created_at berfungsi  
- [ ] Image Icon dan Image Cover tampil  

---
